		...
		if ( isset( $_POST['action'] ) && 'calc_contact_form' === $_POST['action'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
			...
			if ( ! $email_err ) {
				...
				$fields           = array_map(
					function ( $field ) {
						...
						foreach ( $allowed_fields as $allowed ) {
							if ( ! isset( $value['extra'] ) && strpos( $field['alias'], $allowed ) !== false ) {
				...
				$args = array(
					...
					'totals'         => $params['calcTotals'],
					'email_settings' => $general_settings['email_templates'],
					'files'          => $file_urls,
				);

				$body_client = CCBProTemplate::load( 'admin/email-templates/customer-email-template', $args );
				$body_user   = CCBProTemplate::load( 'admin/email-templates/owner-email-template', $args );

				$to_user_email   = wp_mail( $user_email, $subject, $body_user, $headers, $attachments );
				$to_client_email = wp_mail( $client_email, $subject, $body_client, $headers, $attachments );
